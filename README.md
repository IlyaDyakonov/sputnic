...
6. Сделал рефакторинг backend без изменения бизнес-логики: `app.py` остался точкой композиции FastAPI, маршруты вынесены в `api/routes`, DB-сессия приходит через `Depends`, use-cases не зависят от FastAPI, а файловое хранилище подключается через явную абстракцию.
   Почему это лучше: код стал предсказуемее по зонам ответственности - бизнес-правила лежат отдельно от инфраструктуры, а сценарии работы (use-cases) отдельно от роутов и SQL-деталей. Зависимости теперь собираются на границах приложения, поэтому бизнес-логику проще тестировать и менять без HTTP/Celery-деталей.
